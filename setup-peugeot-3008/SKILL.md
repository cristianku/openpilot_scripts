...
For both variants, resolve the complete source commit associated with the matching upstream `release-mici`, recreate the branch from it, apply Cristian's matching opendbc pointer, and push with `--force-with-lease`. Use `git_src_commit` when present and fall back to sunnypilot's matching `v<version>` source tag. Use `commaai/openpilot` for `comma` and `sunnypilot/sunnypilot` for `sunny`.
